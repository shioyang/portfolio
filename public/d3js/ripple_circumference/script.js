...
function main(){
   ...
   svg.selectAll("circle").data(data_mainCircle).enter()
      ...
      .style("fill", function(d){ return vGlobal.genColors(d.id); })
      .style("stroke", function(d){ return vGlobal.genColors(d.id); })
...
function dragstarted(d){
   // reset state
   vGlobal.isInclude = false;
   vGlobal.isOverlay = false;
...
function dragged(d){
   ...
   c_sub.style("fill", function(d){
      vGlobal.isInclude = isInclude(d_sub.cx, d_sub.cy, d_sub.r, d_main.cx, d_main.cy, d_main.r);
      vGlobal.isOverlay = isOverlay(d_sub.cx, d_sub.cy, d_sub.r, d_main.cx, d_main.cy, d_main.r);
      return (!vGlobal.isInclude && vGlobal.isOverlay) ? "#58acfa" : vGlobal.genColors(d.id);
   });
}

function dragended(d){
   var c_sub = d3.select(this);
   if(!vGlobal.isInclude && vGlobal.isOverlay){
      // animation: sub
		c_sub.transition().duration(600)
...
function isOverlay(c1_cx, c1_cy, c1_r, c2_cx, c2_cy, c2_r){
	var dist = Math.sqrt(Math.pow(c1_cx - c2_cx, 2) + Math.pow(c1_cy - c2_cy, 2));
